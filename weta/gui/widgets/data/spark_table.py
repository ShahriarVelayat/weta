...
class OWSparkSQLTableContext(SparkEnvironment, widget.OWWidget):
    ...
    def __init__(self):
        ...
        self.gui_parameters['database'] = ParameterWidget(parent_widget = box, list_values = self.databases, label ='Database', default_value = default_value,
                                                          callback_func = self.refresh_database)

        default_value = self.saved_gui_params.get('table', '')
        self.gui_parameters['table'] = ParameterWidget(parent_widget = box, label ='Table', default_value = default_value, list_values = [default_value])
